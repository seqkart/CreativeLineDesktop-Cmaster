        ...
        private void BtnAdd_Click(object sender, EventArgs e)
        {
            if (btnAdd.Enabled)
            {
                ...
                if (GlobalVariables.ProgCode == "PROG223")
                {
                    ...
                    { S1 = btnAdd.Text, Text = "GST SALE DIFF DOC Addition" };
                    frm.StartPosition = FormStartPosition.CenterScreen;
                    frm.ShowDialog(Parent);
                }
        ...
        private void BtnEdit_Click(object sender, EventArgs e)
        {
            if (btnEdit.Enabled)
            {
                ...
                if (GlobalVariables.ProgCode == "PROG223")
                {
                    DataRow CurrentRow = InvoiceGridView.GetDataRow(InvoiceGridView.FocusedRowHandle);
                    Transaction.FrmImportSaleGSTDiffData frm = new Transaction.FrmImportSaleGSTDiffData()
                    { S1 = btnEdit.Text, Text = "GST SALE DIFF DOC Edition" };
                    frm.StartPosition = FormStartPosition.CenterScreen;
                    frm.DocNo = CurrentRow["DocNo"].ToString();
                    frm.DocDate = Convert.ToDateTime(CurrentRow["DocDate"]);
                    frm.ShowDialog(Parent);
                }
                if (GlobalVariables.ProgCode == "PROG245")
                {
        ...
        private void InvoiceGridView_PopupMenuShowing(object sender, PopupMenuShowingEventArgs e)
        {
            try
            {
                ...
                if (GlobalVariables.ProgCode == "PROG131")
                {
                    ...
                    e.Menu.Items.Add(new DevExpress.Utils.Menu.DXMenuItem("Send Pdf Message", (o1, e1) =>
                    {
                        foreach (DataRow dr in (InvoiceGrid.DataSource as DataTable).Rows)
                        {
                            if (dr["Select"].ToString().ToUpper() == "TRUE")
                ...
                if (GlobalVariables.ProgCode == "PROG131" || GlobalVariables.ProgCode == "PROG141")
                {
                    ...
                    {
                        e.Menu.Items
                            .Add(new DevExpress.Utils.Menu.DXMenuItem("Print Credit Note",
                                                                      (o1, e1) =>
                                                                      {
                                                                          DataRow currentrow = InvoiceGridView.GetDataRow(InvoiceGridView.FocusedRowHandle);
                                                                          MakePrintGrid();
