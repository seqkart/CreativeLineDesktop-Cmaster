...
namespace WindowsFormsApplication1
{
    public partial class frmMaster : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            if (btnAdd.Enabled)
            {
                DataSet dsFormToCall = ProjectFunctions.GetDataSet("Select ProgSubMainForm,ProgDesc from ProgramMaster Where ProgCode='" + GlobalVariables.ProgCode + "'");
                if (dsFormToCall.Tables[0].Rows[0][0].ToString().Trim().Length > 0)
                {

                    string FormToCall = dsFormToCall.Tables[0].Rows[0]["ProgSubMainForm"].ToString();
                    var frm = System.Reflection.Assembly.GetExecutingAssembly().CreateInstance(FormToCall);
                    (frm as DevExpress.XtraEditors.XtraForm).Text = dsFormToCall.Tables[0].Rows[0]["ProgSubMainForm"].ToString() + " Addition";
                    (frm as DevExpress.XtraEditors.XtraForm).StartPosition = FormStartPosition.CenterScreen;
                    (frm as DevExpress.XtraEditors.XtraForm).ShowDialog(Parent);
           //         (frm as DevExpress.XtraEditors.XtraForm).
