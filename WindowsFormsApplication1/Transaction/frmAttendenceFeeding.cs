                    ...
                    if (AttendenceGridView.DataSource != null)
                    {
...

                        {
                            if (AttendenceGridView.FocusedColumn.FieldName == "attendance_in_last" || AttendenceGridView.FocusedColumn.FieldName == "attendance_out_last" || AttendenceGridView.FocusedColumn.FieldName == "attendence_out_night" || AttendenceGridView.FocusedColumn.FieldName == "attendence_in_night")
                            {
                                ...
                                double OvertimeMinutes = TotalMinutes;
                                if (DeductLunch > 0)
                                {
                            ...
                            //if (AttendenceGridView.FocusedColumn.FieldName == "attendance_in_last" || AttendenceGridView.FocusedColumn.FieldName == "attendance_out_last")
                            //{
                            //    DateTime startTime = Convert.ToDateTime(currentrow["attendance_in_last"].ToString());
                            //    DateTime endTime = Convert.ToDateTime(currentrow["attendance_out_last"].ToString());
                            //    TimeSpan spanattendence = endTime.Subtract(startTime);
        ...
        private void Calculate_Click(object sender, EventArgs e)
        {
            foreach (DataRow dr in dt.Rows)
            {
                if (dr["status_code"].ToString() == "PP")
                {
                    ...
                    DateTime startTime = Convert.ToDateTime(dr["attendance_in_first"].ToString());
                    DateTime endTime = Convert.ToDateTime(dr["attendance_out_last"].ToString());
                    TimeSpan spanattendence = endTime.Subtract(startTime);
                    double TotalMinutes = spanattendence.TotalMinutes;
                    DateTime lunchstartTime = Convert.ToDateTime(dr["attendance_out_first"].ToString());
                    DateTime lunchendTime = Convert.ToDateTime(dr["attendance_in_last"].ToString());
                    TimeSpan spanbreak = lunchendTime.Subtract(lunchstartTime);
                    ...
                    {
                        DateTime overtimestartTime = Convert.ToDateTime(dr["attendence_in_night"].ToString());
                        DateTime overtimeendTime = Convert.ToDateTime(dr["attendence_out_night"].ToString());
                        if (overtimestartTime.Hour + overtimeendTime.Hour > 24)
                        {
                            overtimeendTime = Convert.ToDateTime(dr["attendence_out_night"].ToString()).AddDays(1);
                        }
                        else
                        {
                ...
                if (dr["status_code"].ToString() == "RR" || dr["status_code"].ToString() == "HH")
                {
                    ...
                    if (dr["attendance_in_first"].ToString().Length > 0 || dr["attendance_out_first"].ToString().Length > 0)
                    {
                        ...
                        {
                            DateTime startTime = Convert.ToDateTime(dr["attendance_in_first"].ToString());
                            DateTime endTime = Convert.ToDateTime(dr["attendance_out_first"].ToString());
                            TimeSpan spanattendence = endTime.Subtract(startTime);
                            TotalMinutes = spanattendence.TotalMinutes;
                        }
                        OvertimeMinutes = TotalMinutes;
                ...
                if (dr["status_code"].ToString() == "HA")
                {
                    ...
                    DateTime startTime = Convert.ToDateTime(dr["attendance_in_last"].ToString());
                    DateTime endTime = Convert.ToDateTime(dr["attendance_out_last"].ToString());
                    TimeSpan spanattendence = endTime.Subtract(startTime);
                    double TotalMinutes = spanattendence.TotalMinutes;
                    //DateTime lunchstartTime = Convert.ToDateTime(dr["attendance_out_first"].ToString());
